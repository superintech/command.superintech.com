...
export default function ProjectDetailPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold text-[hsl(var(--text-secondary))] bg-[hsl(var(--layout-card-hover))]">
            {status}
          </span>
        );
    }
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      ...
      default:
        return <span className="px-2 py-0.5 rounded text-[10px] font-bold text-[hsl(var(--text-secondary))] bg-[hsl(var(--layout-card-hover))]">LOW</span>;
    }
  };
  ...
  const getTaskStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <span className="px-2 py-0.5 rounded text-[10px] font-bold text-[hsl(var(--text-secondary))] bg-[hsl(var(--layout-card-hover))]">{status.replace('_', ' ')}</span>;
    }
  };
  ...
  return (
    <div className="min-h-full space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="bg-[hsl(var(--layout-card))] rounded-xl p-4 border border-[hsl(var(--layout-border))]">
          <div className="flex items-center gap-3">
        ...
        <div className="bg-[hsl(var(--layout-card))] rounded-xl p-4 border border-[hsl(var(--layout-border))]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[hsl(var(--layout-card-hover))] flex items-center justify-center">
      ...
      <Tabs defaultValue="tasks" className="space-y-6">
        <TabsList className="bg-[hsl(var(--layout-card))] border border-[hsl(var(--layout-border))] p-1">
          ...
          {tasks.length === 0 ? (
            ...
            <div className="space-y-2">
              {tasks.map((task) => (
                <div
                  key={task.id}
                  className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))] p-4 cursor-pointer hover:bg-[hsl(var(--layout-card-hover))] transition-colors"
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {(task as any).tags?.length > 0 && (
                          <div className="flex gap-1">
                            ...
                            {(task as any).tags.length > 3 && <span className="text-xs text-[hsl(var(--text-muted))]">+{(task as any).tags.length - 3}</span>}
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <TabsContent value="members" className="space-y-4">
          ...
          {showAddMember && (
            <div className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))] p-4">
              <div className="flex gap-2">
                <select
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {project.members?.map((member) => (
              <div key={member.user.id} className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))] p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
        ...
        <TabsContent value="tags" className="space-y-4">
          ...
          {tags.length === 0 ? (
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {tags.map((tag) => (
                <div key={tag.id} className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))] p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
